...
public class StorageGroupProcessor {
    ...
    private IMergeFileSelector getMergeFileSelector(long budget, MergeResource resource) {
        MergeFileStrategy strategy = IoTDBDescriptor.getInstance().getConfig().getMergeFileStrategy();
        return strategy.getFileSelector(resource, budget);
    }
    ...
    private void handleSqueezeMerge(
        ...
        try {
            ...
            TreeSet<TsFileResource> newSeqFileTreeSet = new TreeSet<>(
                    (o1, o2) -> {
                        int rangeCompare = o1.getFile().getParentFile().getName()
                                .compareTo(o2.getFile().getParentFile().getName());
                        return rangeCompare == 0 ? compareFileName(o1.getFile(), o2.getFile()) : rangeCompare;
                    });
            List<TsFileResource> sequenceFileList = new ArrayList<>(sequenceFileTreeSet);
            int startIdx = sequenceFileList.indexOf(seqFiles.get(0));
            int endIdx = sequenceFileList.indexOf(seqFiles.get(seqFiles.size() - 1));
            for (int i = 0; i < startIdx; i++) {
                newSeqFileTreeSet.add(sequenceFileList.get(i));
            }
            newSeqFileTreeSet.add(newFile);
            for (int i = endIdx + 1; i < sequenceFileList.size(); i++) {
                newSeqFileTreeSet.add(sequenceFileList.get(i));
            }
            sequenceFileTreeSet = newSeqFileTreeSet;
